 ...
 * 2015年11月10日
 */
public abstract class IGraceFullyFrameBase extends JFrame implements IGraceFulTitlePanelHandler, IGraceFulIconInfoProvider
{
    private static final long serialVersionUID = 1L;
    
    protected String title;
    protected ICON_STYLE barSize;
    protected int barStyle = BarIconType.BANNER.getType() | BarIconType.MIN.getType() | BarIconType.CLOSE.getType();
	protected IGraceFullyFrameBase(String title, ICON_STYLE barSize)
    {
		this.title = title;
	...
	protected IGraceFullyFrameBase(String title, ICON_STYLE barSize, int barStyle)
    {
		this.title = title;
		this.barSize = barSize;
	...
	public void moveWindowInit()
	{
		...
		this.getRootPane().addMouseMotionListener(new MouseMotionAdapter() {
			public void mouseDragged(MouseEvent e) {
				Point p = IGraceFullyFrameBase.this.getLocation();
				IGraceFullyFrameBase.this.setLocation(p.x + e.getX() - origin.x, p.y + e.getY()- origin.y);
			}
		});
	}
